...
The Master remains the decision router, and `handoff.ps1` never deploys. The guarded
`master-apply` and `review-apply` commands may update only local `AI_HANDOFF.md` after a
verified read-only Codex capture; they run no git or release action. `cycle` and the
default `loop` still automate only approved Claude Code Implementer turns. For one
explicitly authorized session, `loop -IncludeMaster` and `loop -IncludeReviewer` may
also run the verified Codex Master and Reviewer capture/apply paths. Those flags do not
change the registry's default `AutoLoopEligible` values, and User turns are never
automated.
